...
void IthoFanHub::send_command(IthoCommand command) {
  ...
  switch (command) {
    case IthoCommand::SPEED_LOW:
      set_state(1, 0, "System");
      break;
    case IthoCommand::SPEED_MEDIUM:
      set_state(2, 0, "System");
      break;
    case IthoCommand::SPEED_HIGH:
      set_state(3, 0, "System");
      break;
    case IthoCommand::SPEED_FULL:
...
IthoCommand IthoFanHub::decode_packet(const std::vector<uint8_t> &packet, std::string &device_id) {
  ...
  for (size_t i = 0; i < packet.size() - 6; i++) {
    if (memcmp(&packet[i], ITHO_MSG_LOW, 6) == 0) return IthoCommand::SPEED_LOW;
    if (memcmp(&packet[i], ITHO_MSG_MEDIUM, 6) == 0) return IthoCommand::SPEED_MEDIUM;
    if (memcmp(&packet[i], ITHO_MSG_HIGH, 6) == 0) return IthoCommand::SPEED_HIGH;
    if (memcmp(&packet[i], ITHO_MSG_FULL, 6) == 0) return IthoCommand::SPEED_FULL;
...
void IthoFan::control(const fan::FanCall &call) {
  ...
  if (call.get_state().has_value()) {
    ...
    if (!this->state) {
      // Turn off = set to low
      hub_->send_command(IthoCommand::SPEED_LOW);
    }
  }
  ...
  if (call.get_speed().has_value()) {
    ...
    switch (speed) {
      case 1:
        ...
        break;
      case 2:
        hub_->send_command(IthoCommand::SPEED_MEDIUM);
        break;
      case 3:
        hub_->send_command(IthoCommand::SPEED_HIGH);
        break;
      case 4:
        hub_->send_command(IthoCommand::SPEED_FULL);
        break;
    }
  }
